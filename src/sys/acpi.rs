...
pub fn init() {
    let res = unsafe { AcpiTables::search_for_rsdp_bios(CatosAcpiHandler) };
    match res {
        Ok(acpi) => {
            ...
            if let Ok(dsdt) = &acpi.dsdt() {
                ...
                let handler = Box::new(CatosAmlHandler);
...
#[derive(Clone)]
pub struct CatosAcpiHandler;

impl AcpiHandler for CatosAcpiHandler {
...
struct CatosAmlHandler;

impl Handler for CatosAmlHandler {
